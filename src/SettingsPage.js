                                ...
                                <p>
                                    <input type={"checkbox"} onChange={()=>{
                                        ...
                                        axios.post("http://127.0.0.1:8989/change-setting",data)
                                            .then((response)=>{
                                                console.log("in changr "+response.data)
                                                this.getUserOrganizations();

                                            })
                                    }} value={organization.id} checked={this.doseOrganizationBelongToUser(organization.id)}/><label>{organization.organizationName}</label>
                        </p>
                        </div>
                    )})
                }
